...
var GO = images.read("./ORCImg/GO.png")
...
function firstdialog() {
    if (id("contentPanel").exists()) {
        ...
        sleep(1000);
    }else if (textContains("允许").exists()) {
...
function store() {
    ...
    if (text("浏览店铺").exists()) {
        ...
        if (text("进店任务").exists()) {
            ...
            sleep(2500);
            if (id("nd").exists()) {
                idContains("nd").findOne().click();
                console.log("nd");
            }else if (textContains("继续").exists()) {
                textContains("继续").findOne().click();
...
function saveimg(imgName,imgs) {
    let res =http.get("www.zhanzirui.com/" + imgName + ".png");
    if (res.statusCode == 200) {
        let img = images.fromBytes(res.body.bytes());
        var s=images.save(img, "./ORCImg/" +imgs+ ".png", "png", 50);
        media.scanFile("./ORCImg/" +imgs+ ".png");   //刷新图片文件
    }
}
function 找图(icon, name, ms, numw,numh) {
    try {
        if (ms == null || ms == undefined || ms == ' ') {
            ms = 0;
        }
        if (numw == null || numw == undefined || numw == ' ') {
            numw = 0;
        }
        if (numh == null || numh == undefined || numh == ' ') {
            numh = 0;
        }
        let p = findImage(captureScreen(), icon, {
            region: [0, 0, width, height],
            threshold: 0
        });
        if (p) {
            console.log("点击: " + name + p);
            click(p.x + numw, p.y + numh);
            sleep(ms);
        } else {
            console.log(name + "没有找到");
        }
    } catch (e) {
        console.log("找图异常：" + e);
    }
